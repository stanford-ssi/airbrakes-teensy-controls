...
SensorWeights SensorWeighting::getWeights(uint8_t flight_state, float velocity,
    ...
    case IGNITION:
        // Motor burning: high accelerations, low-g saturates at ±16g
        w.R_accel_low  = 1000.0f; // Saturated, don't trust
        w.R_accel_high = 1.0f;    // Trust high-g
        w.R_baro       = 5.0f;    // Moderate trust
        break;

    case ASCENT:
        // Dynamic weighting based on Mach number and velocity
        if (mach > RocketConfig::MACH_TRANSONIC_LOW &&
            mach < RocketConfig::MACH_TRANSONIC_HIGH) {
            // Transonic: shock effects corrupt pressure readings
            w.R_accel_low  = 1.0f;
            w.R_accel_high = 2.0f;
            w.R_baro       = 500.0f; // Don't trust baro in transonic
